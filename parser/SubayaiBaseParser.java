...
/**
 * Base parser class that handles common Subayai behaviour such as error reporting, scoping and literal
 * parsing.
 */
public abstract class SubayaiBaseParser extends SubayaiBaseVisitor<Void> {

    /**
     * Base implementation of parsing, which handles lexer and parser setup, and error reporting.
     */
    protected static void parseSubayaiImpl(Source source, SubayaiBaseParser visitor) {
        SubayaiLexer lexer = new SubayaiLexer(CharStreams.fromString(source.getCharacters().toString()));
        SubayaiParser parser = new SubayaiParser(new CommonTokenStream(lexer));
        ...
        parser.subayai().accept(visitor);
    }

    @Override
    public Void visitPkg_declaration(SubayaiParser.Pkg_declarationContext ctx) {
        return null;
    }

    protected final SubayaiLanguage language;
    protected final Source source;
    protected final TruffleString sourceString;

    protected SubayaiBaseParser(SubayaiLanguage language, Source source) {
        this.language = language;
        this.source = source;
        sourceString = SubayaiStrings.fromJavaString(source.getCharacters().toString());
    }
    ...
    private static void throwParseError(Source source, int line, int charPositionInLine, Token token, String message) {
        ...
        throw new SubayaiParseError(source, line, col, length, String.format("Error(s) parsing script:%n" + location + message));
    }
    ...
    protected TruffleString asTruffleString(Token literalToken, boolean removeQuotes) {
        ...
        return sourceString.substringByteIndexUncached(fromIndex * 2, length * 2, SubayaiLanguage.STRING_ENCODING, true);
    }

    // ------------------------------- locals handling --------------------------

    private static class FindLocalsVisitor extends SubayaiBaseVisitor<Void> {
