...
/**
 * Subayai does not need a sophisticated error checking and reporting mechanism, so all unexpected
 * conditions just abort execution. The exceptions defined in this class are used when we abort from
 * within the Subayai implementation.
 */
@SuppressWarnings("serial")
public final class SubayaiException extends AbstractTruffleException {

    private static final InteropLibrary UNCACHED_LIB = InteropLibrary.getFactory().getUncached();

    SubayaiException(String message, Node location) {
        super(message, location);
    }

    @TruffleBoundary
    public static AbstractTruffleException create(String message, Node location) {
        return new SubayaiException(message, location);
    }
    ...
    public static AbstractTruffleException typeError(Node operation, Object... values) {
        ...
        if (operation != null) {
            NodeInfo nodeInfo = SubayaiLanguage.lookupNodeInfo(operation.getClass());
            if (nodeInfo != null) {
    ...
    /**
     * Provides a user-readable message for run-time type errors. Subayai is strongly typed, i.e., there
    ...
    public static AbstractTruffleException typeError(Node location, String operationName, Object... values) {
        ...
        AbstractTruffleException ex = SubayaiException.create("", location);
        if (location != null) {
        ...
        for (int i = 0; i < values.length; i++) {
             ...
             */
            Object value = SubayaiLanguageView.forValue(values[i]);
            result.append(sep);
            sep = ", ";
            if (value == null) {
                result.append("ANY");
        ...
        return SubayaiException.create(result.toString(), location);
    }
