...
/**
 * Represents a Subayai function. On the Truffle level, a callable element is represented by a
 * {@link RootCallTarget call target}. This class encapsulates a call target, and adds version
 * support: functions in Subayai can be redefined, i.e. changed at run time. When a function is
 ...
 * <p>
 * The {@link #callTarget} can be {@code null}. To ensure that only one {@link SubayaiFunction} instance
 * per name exists, the {@link SubayaiFunctionRegistry} creates an instance also when performing name
 * lookup. A function that has been looked up, i.e., used, but not defined, has a call target that
 * encapsulates a {@link SubayaiUndefinedFunctionRootNode}.
 */
@ExportLibrary(InteropLibrary.class)
@SuppressWarnings("static-method")
public final class SubayaiFunction implements TruffleObject {

    public static final int INLINE_CACHE_SIZE = 2;

    private static final TruffleLogger LOG = TruffleLogger.getLogger(SubayaiLanguage.ID, SubayaiFunction.class);
    ...
    protected SubayaiFunction(SubayaiLanguage language, TruffleString name) {
        this(name, language.getOrCreateUndefinedFunction(name));
    }

    protected SubayaiFunction(TruffleString name, RootCallTarget callTarget) {
        this.name = name;
     ...
     * changing it has an effect on Subayai programs.
     */
    @Override
    ...
    /**
     * {@link SubayaiFunction} instances are always visible as executable to other languages.
     */
    @SuppressWarnings("static-method")
    @ExportMessage
    @TruffleBoundary
    SourceSection getSourceLocation() {
        return ((SubayaiRootNode) getCallTarget().getRootNode()).ensureSourceSection();
    }
    ...
    static final class IsIdenticalOrUndefined {
        @Specialization
        static TriState doSubayaiFunction(SubayaiFunction receiver, SubayaiFunction other) {
            /*
             * SubayaiFunctions are potentially identical to other SubayaiFunctions.
             */
            return receiver == other ? TriState.TRUE : TriState.FALSE;
        }

        @Fallback
        static TriState doOther(SubayaiFunction receiver, Object other) {
    ...
    @ExportMessage
    @TruffleBoundary
    static int identityHashCode(SubayaiFunction receiver) {
    ...
    abstract static class Execute {
         ...
         *
         * <p>
         * Since Subayai is a quite simple language, the benefit of the inline cache seems small: after
         ...
         * target is usually much more complicated than in Subayai.
         * </p>
         *
         * <p>
         ...
         * function is redefined, the call target maintained by the SubayaiFunction object is changed. To
         * avoid a check for that, we use an Assumption that is invalidated by the SubayaiFunction when
         * the change is performed. Since checking an assumption is a no-op in compiled code, the
         * assumption check performed by the DSubayai does not add any overhead during optimized
         * execution.
         * </p>
         *
         * @see Cached
        ...
        @SuppressWarnings("unused")
        protected static Object doDirect(SubayaiFunction function, Object[] arguments,
        ...
        protected static Object doIndirect(SubayaiFunction function, Object[] arguments,
