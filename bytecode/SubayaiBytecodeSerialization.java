...
public final class SubayaiBytecodeSerialization {

    private static final byte CODE_Subayai_NULL = 0;
    ...
    private SubayaiBytecodeSerialization() {
        // no instances
    }

    public static byte[] serializeNodes(BytecodeParser<SubayaiBytecodeRootNodeGen.Builder> parser) throws IOException {
        ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream();
        DataOutputStream outputStream = new DataOutputStream(byteArrayOutputStream);

        SubayaiBytecodeRootNodeGen.serialize(outputStream, (context, buffer, object) -> {
            if (object instanceof SubayaiNull) {
                buffer.writeByte(CODE_Subayai_NULL);
            ...
            } else if (object instanceof SubayaiBigInteger) {
                SubayaiBigInteger num = (SubayaiBigInteger) object;
    ...
    static void writeString(DataOutput buffer, TruffleString str) throws IOException {
        writeByteArray(buffer, str.getInternalByteArrayUncached(SubayaiLanguage.STRING_ENCODING).getArray());
    }
    ...
    public static BytecodeRootNodes<SubayaiBytecodeRootNode> deserializeNodes(SubayaiLanguage language, byte[] inputData) throws IOException {
        Supplier<DataInput> input = () -> SerializationUtils.createDataInput(ByteBuffer.wrap(inputData));
        return SubayaiBytecodeRootNodeGen.deserialize(language, BytecodeConfig.DEFAULT, input, (context, buffer) -> {
            byte tag;
            switch (tag = buffer.readByte()) {
                case CODE_Subayai_NULL:
                ...
                case CODE_BIG_INT:
                    return new SubayaiBigInteger(new BigInteger(readByteArray(buffer)));
                case CODE_SOURCE: {
                    String name = new String(readByteArray(buffer));
                    return Source.newBuilder(SubayaiLanguage.ID, "", name).build();
                }
                default:
    ...
    static TruffleString readString(DataInput buffer) throws IOException {
        return TruffleString.fromByteArrayUncached(readByteArray(buffer), SubayaiLanguage.STRING_ENCODING);
    }
}
