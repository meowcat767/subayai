...
final class SubayaiBytecodeScopeExports {

    @ExportMessage
    static boolean hasRootInstance(TagTreeNode node, Frame frame, @Bind SubayaiContext context) {
        return getRootInstanceSlowPath(context, node) != null;
    }

    @ExportMessage
    static Object getRootInstance(TagTreeNode node, Frame frame, @Bind SubayaiContext context) throws UnsupportedMessageException {
        // The instance of the current RootNode is a function of the same name.
        return getRootInstanceSlowPath(context, node);
    }

    @TruffleBoundary
    private static Object getRootInstanceSlowPath(SubayaiContext context, TagTreeNode node) {
        return context.getFunctionRegistry().getFunction(SubayaiStrings.getSubayaiRootName(node.getRootNode()));
    }
    ...
    static final class ArgumentsScope implements TruffleObject {
        ...
        @NeverDefault final SubayaiBytecodeRootNode rootNode;
        final Frame frame;
        private Map<String, Integer> nameToIndex;

        private ArgumentsScope(TagTreeNode node, Frame frame) {
            this.rootNode = (SubayaiBytecodeRootNode) node.getBytecodeNode().getBytecodeRootNode();
            this.frame = frame;
        }
        ...
        boolean accepts(@Cached(value = "this.rootNode", adopt = false) SubayaiBytecodeRootNode cachedRootNode) {
