 ...
 * SOFTWARE.
 */
package space.meowcats.subayai.nodes.controlflow;
...
import space.meowcats.subayai.nodes.local.SubayaiScopedNode;
...
public final class SubayaiBlockNode extends SubayaiStatementNode implements BlockNode.ElementExecutor<SubayaiStatementNode> {
     ...
     * bailout.
     */
    @Child private BlockNode<SubayaiStatementNode> block;
     ...
     */
    @CompilationFinal(dimensions = 1) private SubayaiWriteLocalVariableNode[] writeNodesCache;
    ...
    public SubayaiBlockNode(SubayaiStatementNode[] bodyNodes) {
        /*
     ...
     * {@link SubayaiStatementNode#executeVoid} method of all children to be inlined.
     */
    @Override
    ...
    public List<SubayaiStatementNode> getStatements() {
        if (block == null) {
     ...
     * startsWith semantics if needed. For Subayai we don't need to pass any argument as we just have
     ...
     */
    @Override
    public void executeVoid(VirtualFrame frame, SubayaiStatementNode node, int index, int argument) {
     ...
     * are included.
     */
    public SubayaiWriteLocalVariableNode[] getDeclaredLocalVariables() {
        SubayaiWriteLocalVariableNode[] writeNodes = writeNodesCache;
        if (writeNodes == null) {
    ...
    private SubayaiWriteLocalVariableNode[] findDeclaredLocalVariables() {
        if (block == null) {
            return new SubayaiWriteLocalVariableNode[]{};
        }
        // Search for those write nodes, which declare variables
        List<SubayaiWriteLocalVariableNode> writeNodes = new ArrayList<>(4);
        ...
        NodeUtil.forEachChild(block, new NodeVisitor() {
            ...
            public boolean visit(Node node) {
                ...
                if (node instanceof SubayaiScopedNode) {
                    SubayaiScopedNode scopedNode = (SubayaiScopedNode) node;
                    scopedNode.setVisibleVariablesIndexOnEnter(varsIndex[0]);
                }
                // Do not enter any nested blocks.
                if (!(node instanceof SubayaiBlockNode)) {
                    NodeUtil.forEachChild(node, this);
                }
                // Write to a variable is a declaration unless it exists already in a parent scope.
                if (node instanceof SubayaiWriteLocalVariableNode) {
                    SubayaiWriteLocalVariableNode wn = (SubayaiWriteLocalVariableNode) node;
                    if (wn.isDeclaration()) {
                        writeNodes.add(wn);
                        varsIndex[0]++;
                    }
        ...
        SubayaiWriteLocalVariableNode[] parentVariables = null;
        if (parentBlock instanceof SubayaiBlockNode) {
            parentVariables = ((SubayaiBlockNode) parentBlock).getDeclaredLocalVariables();
        }
        SubayaiWriteLocalVariableNode[] variables = writeNodes.toArray(new SubayaiWriteLocalVariableNode[writeNodes.size()]);
        parentBlockIndex = variables.length;
        if (parentVariables == null || parentVariables.length == 0) {
            return variables;
        } else {
            int parentVariablesIndex = ((SubayaiBlockNode) parentBlock).getParentBlockIndex();
            ...
            SubayaiWriteLocalVariableNode[] allVariables = Arrays.copyOf(variables, allVarsLength);
