 ...
 * "evaluating" source code in Subayai.</li>
 * <li>Conversion of arguments to types understood by Subayai. The Subayai source code can be evaluated from a
 * different language, i.e., the caller can be a node from a different language that uses types not
 * understood by Subayai.</li>
 * </ul>
 */
public final class SubayaiEvalRootNode extends RootNode {

    private static final TruffleString ROOT_EVAL = SubayaiStrings.constant("root eval");
    ...
    public SubayaiEvalRootNode(SubayaiLanguage language, RootCallTarget rootFunction, Map<TruffleString, RootCallTarget> functions) {
        super(language);
    ...
    public Object execute(VirtualFrame frame) {
        if (language.isSingleContext()) {
             ...
             * multi-context case is initialized with SubayaiLanguage#initializeMultipleContexts. That
        ...
        if (mainCallNode == null) {
            ...
            /* Conversion of arguments to types understood by Subayai. */
            Object[] arguments = frame.getArguments();
            for (int i = 0; i < arguments.length; i++) {
                arguments[i] = SubayaiContext.fromForeignValue(arguments[i]);
            }
    ...
    private void registerFunctions() {
        SubayaiContext.get(this).getFunctionRegistry().register(functions);
    }
