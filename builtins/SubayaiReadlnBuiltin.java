...
/**
 * Builtin function that reads a String from the {@link SubayaiContext#getInput() standard input}.
 */
@NodeInfo(shortName = "readln")
public abstract class SubayaiReadlnBuiltin extends SubayaiBuiltinNode {

    @Specialization
    public TruffleString readln(@Cached TruffleString.FromJavaStringNode fromJavaStringNode,
                    @Bind SubayaiContext context) {
        TruffleString result = fromJavaStringNode.execute(doRead(context.getInput()), SubayaiLanguage.STRING_ENCODING);
        if (result == null) {
            /*
             ...
             */
            result = SubayaiStrings.EMPTY_STRING;
        }
        return result;
    }
    ...
    private String doRead(BufferedReader in) {
        try {
            ...
            throw SubayaiException.create(ex.getMessage(), this);
        }
    }
}
