...
public abstract class SubayaiHelloEqualsWorldBuiltin extends SubayaiBuiltinNode {

    @Specialization
    @TruffleBoundary
    public TruffleString change() {
        return Truffle.getRuntime().iterateFrames((f) -> {
            SubayaiRootNode root = (SubayaiRootNode) ((RootCallTarget) f.getCallTarget()).getRootNode();
            Object[] names = root.getLocalNames(f);
            for (int i = 0; i < names.length; i++) {
                Object slotName = names[i];
                if (slotName != null && SubayaiStrings.HELLO.equalsUncached((TruffleString) slotName, SubayaiLanguage.STRING_ENCODING)) {
                    Object[] values = root.getLocalValues(f);
                    values[i] = SubayaiStrings.WORLD;
                    root.setLocalValues(f, values);
                    break;
                }
            }
            return SubayaiStrings.WORLD;
        }, 1);
    }
}
